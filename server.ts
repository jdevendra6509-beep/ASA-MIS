...
async function startServer() {
  ...
  app.post("/api/employees", async (req, res) => {
    ...
    if (role === 'Partner') {
      ...
      reportingManager = reportingPartner; // Manager's reporting manager is same as partner
    }
  ...
  app.post("/api/employees/:id/resend-invite", async (req, res) => {
    const { id } = req.params;
    try {
      ...
      if (user.status !== 'Pending') {
        return res.status(400).json({ error: "Only pending employees can be re-invited" });
      }

      const registrationToken = user.registrationToken || uuidv4();
      const expires = new Date();
      expires.setHours(expires.getHours() + 48);
      ...
      const emailSent = await sendInvitationEmail(user.firstName, user.email, registrationToken);
      if (emailSent) {
        res.json({ success: true });
      } else {
        res.status(503).json({ error: "Outlook integration not active. Please connect Outlook in settings." });
      }
    } catch (error: any) {
      res.status(500).json({ error: error.message });
    }
